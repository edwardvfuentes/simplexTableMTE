...
tablaSimplex <- function(lp, wb){
  ...
  estilocelda <- XLConnect::createCellStyle(problem80es, "estilocelda")
  ...
  while(iteracion <= 4){
    costes <- XLConnect::readNamedRegion(wb, str_c("costes_redux", iteracion), header = FALSE)
    m_restr <- XLConnect::readNamedRegion(wb, str_c("restricciones", iteracion), header = FALSE)
    celda_restr <- coord_celdas(wb, str_c("restricciones", iteracion))
    pivote <- pivot_calc(m_restr, celda_restr, costes)
    base_vars <- XLConnect::readWorksheet(wb, "Sheet1", startRow = inicio + 1, endRow = inicio + nrow(problem_53_es), startCol = 1, endCol = 1, header = FALSE)
    if(!is.null(pivote)){
      #Coloreamos la celda del pivote
      XLConnect::setCellStyle(problem80es, sheet = "Sheet1", row = inicio + pivote$Fila, col = 2 + pivote$Columna, cellstyle = estilocelda)
    ...
    XLConnect::writeWorksheet(wb, base_vars, "Sheet1", startRow = inicio + nrow(prueba) + 5, startCol = 1, header = FALSE)
